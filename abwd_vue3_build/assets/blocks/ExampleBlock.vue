...
<template>
  <h1>Example Block</h1>
  <p class="example-block-message">
    Hello world from inside the example block SFC! This block's ID is
    {{ props.blockId }}. We are styling this paragraph inside the SFC.
  </p>
  ...
  <div v-else>
    ...
    <p>
      Found from Query String:
      {{
        userData.found
          ? 'Yes, user ID matches query string'
          : 'No, using current logged in user'
      }}
    </p>
  </div>
